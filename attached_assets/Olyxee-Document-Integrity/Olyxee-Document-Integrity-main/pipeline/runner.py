"""
ODI Pipeline Runner — Executes the full end-to-end ODI pipeline.
...
def run(document_id: str, file_path: str, content_type: str) -> dict[str, Any]:
    """
    Run the full ODI pipeline on a document.
    Returns a complete PipelineResult dict.
    """
    ...
    if ocr_result.get("error"):
        return _error_result(document_id, run_id, "OCR failed", ocr_result["message"])
    ...
    for f in trust_result.get("risk_factors", []):
        if "critical" in f or "low_conf" in f:
