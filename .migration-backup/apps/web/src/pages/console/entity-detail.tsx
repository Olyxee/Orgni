          ...
          <section>
            ...
            <ul className="space-y-2 bg-card border border-border rounded-lg divide-y divide-border">
              {data.sources.map((s) => (
                <li
                  key={s.sourceId}
                  className="text-sm flex justify-between items-center group p-3"
                >
                  <Link
                    href={`/app/sources/${s.sourceId}`}
                    className="font-medium hover:text-primary transition-colors flex items-center gap-2 truncate max-w-[70%]"
                  >
