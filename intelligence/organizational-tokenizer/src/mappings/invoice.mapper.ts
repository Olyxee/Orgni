 ...
 *
 * Maps a normalized InvoiceExtraction envelope to OrganizationalTokens.
 *
 * Tokens produced:
 *   1. INVOICE_ISSUED     (EVENT) — the act of the vendor issuing the invoice
 *   2. INVOICE_OBLIGATION (STATE) — buyer's outstanding payment obligation
 *   3. INVOICE_LINE_ITEMS (EVENT) — itemized breakdown, if present
 */
...
import { minConfidence, avgConfidence, buildSourceRef } from "./helpers.js";
...
export function mapInvoiceToTokens(env: InvoiceExtraction): OrganizationalToken[] {
    ...
    tokens.push({
        ...
        validTime: {
            from: env.invoiceDate.value,
            ...(env.dueDate ? { to: env.dueDate.value } : {}),
        },
        transactionTime: now,
        scalarValue: {
    ...
    // ── Token 3: INVOICE_LINE_ITEMS — only if present ────────────────────────
    if (env.lineItems.length > 0) {
