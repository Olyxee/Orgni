 ...
 *
 * Tokens produced:
 *   1. CONTRACT_EXECUTED          (EVENT)    — contract signed / executed
 *   2. CONTRACT_COUNTERPARTY      (RELATION) — one per unique party pair
 *   3. CONTRACT_TERMINATION_TERMS (POLICY)   — if termination clause present
 *   4. CONTRACT_CONFIDENTIALITY_TERMS (POLICY) — if confidentiality clause present
 */
...
export function mapContractToTokens(env: ContractExtraction): OrganizationalToken[] {
    ...
    // ── Tokens 2+: CONTRACT_COUNTERPARTY — one per unique party pair ─────────
