      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
        ...
        >
          <div className="mb-6 flex items-center gap-4">
            ...
            <p className="text-sm font-bold tracking-wide text-primary uppercase">Operational Intelligence</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <Button asChild size="lg" className="w-full sm:w-auto h-14 px-8 font-bold shadow-sm rounded-md text-base bg-primary text-primary-foreground hover:bg-primary/90 transition-colors">
              <a href={LOGIN_URL}>
                Request a demonstration
              </a>
            </Button>
            <Button asChild variant="outline" size="lg" className="w-full sm:w-auto h-14 px-8 font-bold border-border text-foreground hover:bg-secondary rounded-md text-base transition-colors">
              <Link href="/platform">
                See how Orgni works
              </Link>
            </Button>
        ...
        >
          <div className="relative w-full aspect-[4/3] md:aspect-auto md:h-[600px] bg-muted rounded-xl overflow-hidden flex items-center justify-center border border-border shadow-lg">
            ...
            <div className="absolute bottom-6 right-6 bg-background/95 backdrop-blur-md p-5 border border-border shadow-xl rounded-md max-w-[260px]">
              <div className="text-[10px] font-mono text-primary font-bold uppercase tracking-widest mb-3 border-b border-border pb-2">Event Stream Live</div>
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  <div className="w-2 h-2 rounded-full bg-primary mt-1 shrink-0 animate-pulse"></div>
                  <div className="text-xs font-mono font-bold text-foreground">SUPPLIER_COMPLIANCE_UPDATED</div>
                </div>
                <div className="flex items-start gap-3 opacity-60">
                  <div className="w-2 h-2 rounded-full bg-muted-foreground mt-1 shrink-0"></div>
                  <div className="text-xs font-mono font-medium text-foreground">INVOICE_MATCHED</div>
                </div>
                <div className="flex items-start gap-3 opacity-40">
                  <div className="w-2 h-2 rounded-full bg-muted-foreground mt-1 shrink-0"></div>
                  <div className="text-xs font-mono font-medium text-foreground">POLICY_VIOLATION_DETECTED</div>
                </div>
              </div>
            </div>
          </div>
