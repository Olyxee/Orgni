        ...
        <div className="rounded-lg border border-border divide-y divide-border overflow-hidden bg-card">
          <table className="w-full text-sm">
            <thead className="bg-muted/30">
              <tr>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground w-1/2">Entity</th>
                <th className="px-4 py-3 text-left font-medium text-muted-foreground">Type / Key</th>
                <th className="px-4 py-3 text-right font-medium text-muted-foreground">Occurrences</th>
              </tr>
            </thead>
              ...
              {entities.map(e => (
                <tr key={e.key} className="hover:bg-muted/10 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/app/entities/${encodeURIComponent(e.key)}`} className="font-medium hover:text-primary transition-colors block">
