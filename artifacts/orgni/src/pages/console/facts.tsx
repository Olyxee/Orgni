    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {facts.length === 0 ? (
        ...
        <div className="space-y-4">
          {facts.map((f, i) => {
            const status = String(f.fact.epistemic_status || f.fact.status || "ASSERTED");
            return (
              <div key={i} className="p-4 rounded-lg border border-border bg-card flex flex-col sm:flex-row gap-4 justify-between sm:items-start">
                <div className="flex-1 min-w-0">
                  <div className="mb-3">
                    <Badge variant="outline" className="font-mono text-[10px] uppercase tracking-wider font-normal bg-muted/30">{status}</Badge>
                  </div>
                ...
                <div className="sm:text-right shrink-0">
                  <div className="text-xs text-muted-foreground flex items-center sm:justify-end gap-1.5 mt-2 sm:mt-0">
                    ...
                    <Link href={`/app/sources/${f.source.sourceId}`} className="hover:underline truncate max-w-[200px]">{f.source.filename}</Link>
                  </div>
