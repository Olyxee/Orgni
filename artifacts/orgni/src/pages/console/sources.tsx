...
export default function Sources() {
  ...
  async function onUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (err.status === 503 || err.code === "document_intelligence_unavailable") {
        setUploadError("Processing services are not yet connected in this environment. Once connected, Orgni will automatically extract entities, facts, and relationships from this source.");
      } else {
          ...
          {sources.map(s => (
            <Link key={s.sourceId} href={`/app/sources/${s.sourceId}`}>
              <div className="px-4 py-4 flex items-center justify-between text-sm hover:bg-muted/30 transition-colors cursor-pointer group">
                <div>
                  <div className="font-medium group-hover:text-primary transition-colors">{s.filename}</div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Badge variant={s.state === 'COMPLETED' ? 'default' : 'outline'} className="font-mono text-xs">
                    {s.state}
                  </Badge>
                </div>
              </div>
