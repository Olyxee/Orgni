...
export default function Relationships() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight mb-2">Relationships</h1>
        <p className="text-muted-foreground text-sm">Mapped connections between organizational entities.</p>
      </div>
          ...
          {relationships.map((r, i) => (
            <div key={i} className="p-4 rounded-lg border border-border bg-card">
              ...
              <div className="mt-4 pt-3 border-t border-border/50 text-xs text-muted-foreground flex items-center gap-1.5">
                ...
                Provenance: <Link href={`/app/sources/${r.source.sourceId}`} className="hover:underline truncate">{r.source.filename}</Link>
              </div>
            </div>
          ))}
        </div>
