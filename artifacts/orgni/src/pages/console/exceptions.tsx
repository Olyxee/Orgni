...
import { AlertTriangle, FileText, XCircle, AlertCircle, Info } from "lucide-react";
...
export default function Exceptions() {
  ...
  const hasExceptions = data.conflicts.length > 0 || data.rejected.length > 0 || data.warnings.length > 0 || data.failedSources.length > 0;
            ...
            <section>
              ...
              <div className="space-y-3">
                {data.failedSources.map(s => (
                  <div key={s.sourceId} className="p-4 border border-destructive/20 bg-destructive/5 rounded-lg">
                    <div className="flex justify-between items-start mb-2">
                      <Link href={`/app/sources/${s.sourceId}`} className="font-medium text-destructive hover:underline">{s.filename}</Link>
                      <span className="text-xs text-muted-foreground font-mono">{new Date(s.uploadedAt).toLocaleString()}</span>
                    </div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {data.conflicts.map((c, i) => (
                  <div key={i} className="p-4 border border-border rounded-lg bg-card">
                    <DefensiveDisplay data={c.conflict as Record<string, unknown>} />
                    <div className="mt-3 text-xs text-muted-foreground flex items-center gap-1.5 pt-3 border-t border-border/50">
                      <FileText className="w-3 h-3 shrink-0" />
                      Source: <Link href={`/app/sources/${c.source.sourceId}`} className="hover:underline truncate">{c.source.filename}</Link>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {data.rejected.length > 0 && (
            <section>
              <h2 className="text-sm font-semibold mb-4 flex items-center gap-2 border-b border-border pb-2">
                <AlertCircle className="w-4 h-4 text-muted-foreground" /> Rejected Assertions
              </h2>
              <div className="space-y-3">
                {data.rejected.map((r, i) => (
                  <div key={i} className="p-4 border border-border rounded-lg bg-card flex justify-between items-start gap-4">
                    <p className="text-sm text-foreground/80 leading-relaxed">{r.reason}</p>
                    <Link href={`/app/sources/${r.source.sourceId}`} className="text-xs text-muted-foreground shrink-0 hover:underline flex items-center gap-1 mt-1">
                      <FileText className="w-3 h-3" /> <span className="truncate max-w-[150px]">{r.source.filename}</span>
                    </Link>
                  </div>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="space-y-2 bg-card border border-border rounded-lg p-1">
                {data.warnings.map((w, i) => (
                  <div key={i} className="p-3 flex justify-between items-center border-b border-border/50 last:border-0 text-sm">
                    <span className="text-muted-foreground">{w.warning}</span>
                    <Link href={`/app/sources/${w.source.sourceId}`} className="text-xs shrink-0 hover:underline flex items-center gap-1 ml-4">
                      <FileText className="w-3 h-3" /> <span className="truncate max-w-[150px]">{w.source.filename}</span>
                    </Link>
                  </div>
                ))}
              </div>
