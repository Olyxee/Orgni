...
import { Check, Copy, Key, Book, Terminal, Layers, FileText, History, AlertTriangle } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mb-10 leading-relaxed">
        Give your agents access to governed identity, relationships, policies, evidence, and operational state through a unified API.
      </p>
    ...
    <section className="py-24 bg-[#050505] text-white px-6">
      <div className="max-w-screen-xl mx-auto">
        <div className="max-w-2xl mb-16">
          <h2 className="text-3xl md:text-4xl font-medium tracking-tight mb-4">What Orgni exposes</h2>
          <p className="text-lg text-white/60 leading-relaxed">
            A single API endpoint that returns the complete operational and historical context an agent needs to make a decision about a resource.
          </p>
        </div>
...
<span className="text-[#ff7b72]">&#125;</span>
          ...
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-10">
            {[
              { icon: Layers, title: "State & Identity", desc: "Canonical entity resolution and current verified state." },
              { icon: FileText, title: "Applicable Rules", desc: "Filtered requirements by entity, role, and jurisdiction." },
              { icon: History, title: "Evidence & History", desc: "Verified history, previous decisions, and source evidence." },
              { icon: AlertTriangle, title: "Conflicts", desc: "Preserved conflicting claims marked for human review." },
            ].map((item, i) => (
              <div key={i}>
                <div className="w-10 h-10 rounded-md bg-white/5 border border-white/10 flex items-center justify-center mb-4">
                  <item.icon className="text-primary" size={18} />
                </div>
                <h3 className="font-medium text-white text-base mb-2">{item.title}</h3>
    ...
    <section className="py-24 px-6 max-w-screen-xl mx-auto border-b border-border">
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <h2 className="text-3xl md:text-4xl font-medium tracking-tight mb-4">Authentication</h2>
          <p className="text-lg text-muted-foreground mb-8 max-w-md">
            All requests to the Orgni API require a Bearer token. You can generate and manage API keys in your developer dashboard.
          </p>
        ...
        <div>
          ...
          >
            <pre className="text-white/80 leading-relaxed text-xs md:text-sm flex-1 font-mono">
<span className="text-[#79c0ff]">GET</span> <span className="text-white/90">/v1/context</span> <span className="text-white/40">HTTP/1.1</span>{"\n"}
<span className="text-[#79c0ff]">Host:</span> <span className="text-white/90">api.orgni.com</span>{"\n"}
<span className="text-[#79c0ff]">Authorization:</span> <span className="text-[#a5d6ff]">Bearer orgni_live_xxxxxxxx</span>
            </pre>
          </CodeBlock>
        </div>
      </div>
...
function NextSteps() {
  return (
    <section className="py-24 px-6 max-w-screen-xl mx-auto">
      <h2 className="text-3xl md:text-4xl font-medium tracking-tight mb-10 text-center lg:text-left">Where to go next</h2>
      <div className="max-w-4xl">
        <Link href="/docs" className="group block p-8 md:p-10 rounded-xl border border-border bg-muted/20 hover:bg-muted/40 hover:border-primary/30 transition-all">
          <div className="w-12 h-12 rounded-md bg-primary/10 flex items-center justify-center mb-6">
            <Book className="text-primary" size={24} />
          </div>
          <h3 className="text-xl font-medium mb-3 group-hover:text-primary transition-colors">Read the Documentation</h3>
          <p className="text-muted-foreground leading-relaxed max-w-2xl">
            Learn the core concepts, read integration guides, and explore the complete REST API documentation. See how to structure your agent to leverage the organisational model.
          </p>
        </Link>
      </div>
    </section>
