...
def map_contract(
    ...
    *,
    ...
    # Deterministic Entity UUID for Contract
    ...
    # Deterministic Entity UUID for Supplier (normalized casing & whitespace)
    ...
    if governing_rule_name is not None:
        # Deterministic Entity UUID for Rule
        rule_id = generate_deterministic_id("rule", governing_rule_name.strip().lower())
        rule = Entity(
            id=rule_id,
            type=EntityType.RULE,
            attributes={"name": governing_rule_name.strip()},
            provenance=prov,
        )
        ...
        result.assertions.append(
            AttributeAssertion(
                entity_id=rule.id, 
                attribute_name="name", 
                value=governing_rule_name.strip(), 
                provenance=prov
            )
        )
        result.relationships.append(
